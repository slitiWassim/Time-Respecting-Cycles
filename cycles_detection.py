...
def time_respecting_cycles(rolling_g, max_length=None, max_cycles=None, max_duration=None):
    ...
    # ---- Build adjacency with sorted edge timestamps ----
    ...
    def strongly_connected_components():
        ...
        for v in list(adjacency.keys()):
            if v not in index:
                dfs(v)

        return sccs

    # ---- Johnson-like temporal cycle search with early duration pruning ----
    def _johnson_cycle_search(start):
        ...
        def backtrack(v, last_time):
            blocked.add((v, last_time))
            closed = False

            for w, t_next in _out_neighbors(v, last_time):
                # ---- Early duration pruning ----
                if times_path and max_duration is not None:
                    if t_next - times_path[0] > max_duration:
                        continue  # skip: would exceed allowed duration

                if w == start:
                    if times_path:
                        first_t = times_path[0]
                        duration = t_next - first_t
                    else:
                        duration = 0
                    ...
                    if (
                        ...
                        if max_cycles and cycle_count[0] >= max_cycles:
                            return
                elif (w, t_next) not in blocked and (max_length is None or len(path) < max_length):
                    path.append(w)
            ...
            if closed:
                ...
                for w, _ in _out_neighbors(v, None):
                    B[(w, None)].add((v, last_time))

            if (v, last_time) in blocked:
                blocked.remove((v, last_time))
